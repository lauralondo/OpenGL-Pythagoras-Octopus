			 ...
			 float n)							//
{
	...
	if( dist > (20.0/WINDOW_SIZE)) { //if the square is larger than 1 pixel
		squares(Q4x, Q4y, newx, newy, n+0.08); //left recursion
...
void display(void)
{
  ...
  squares(-1.6, 5.0, 1.6, 5.0, 0.0); //call he recursive function

  //aaaand just for fun.....

  glColor3f(1.0, 1.0, 1.0);
  glBegin(GL_POLYGON);			//left eye
  glVertex3f(-0.7, 4.2,0.0);
  glVertex3f(-0.4, 4.2,0.0);
  glVertex3f(-0.4, 3.5,0.0);
  glVertex3f(-0.7, 3.5,0.0);
  glEnd();
  ...
  glVertex3f(0.05,  3.2,0.0);
  glVertex3f(-0.05, 3.2,0.0);
  glVertex3f(-0.2, 2.9,0.0);
  glVertex3f(0.2,  2.9,0.0);
  glEnd();
...
int main(int argc, char **argv)
{
  ...
  glutCreateWindow ("Pythagoras OCTOPUS!!!!  -->  Pythagopus??");
